...
class BO_algo:
    def __init__(self):
        ...
        # GP f
        f_kernel = Matern(length_scale=len_scale_f, nu=nu_f)
        self.gp_f = GaussianProcessRegressor(
            kernel=f_kernel, alpha=sigma_f**2, normalize_y=True
        )
        ...
        # GP v
        v_kernel = Matern(length_scale=len_scale_v, nu=nu_v) + ConstantKernel(
            constant_value=mean_v
        )
        self.gp_v = GaussianProcessRegressor(
            kernel=v_kernel, alpha=sigma_v**2, normalize_y=True
        )
    ...
    def acquisition_function(self, x, method="UCB"):
        ...
        if method == "UCB":
            ...
            # constraint
            c_mean, c_std = self.constraint_model.predict(
                np.atleast_2d(x), return_std=True
            )
            prob_constraint = norm.cdf(0, loc=c_mean, scale=c_std)

            # objective
            y_mean, y_std = self.objective_model.predict(
                np.atleast_2d(x), return_std=True
            )
    ...
    def get_solution(self):
        ...
        valid_f[self.V < self.v_min] = -np.inf  # discard all slow results
        optmal_x = self.X[np.argmax(valid_f)]
        print(
            f"valid_f: {valid_f[self.V > self.v_min]} \n \
